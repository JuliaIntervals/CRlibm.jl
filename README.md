...
- trigonometric: `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `sinpi`, `cospi`, `tanpi`, `atanpi`
- hyperbolic: `sinh`, `cosh`

## Installation

The CRlibm.jl package requires to [install Julia](https://julialang.org/downloads/) (v1.3 or above).
...
The library provides correctly-rounded versions of elementary functions, such as `sin` and `exp`. The available rounding modes are `RoundNearest`, `RoundUp`, `RoundDown` and `RoundToZero`. The functions are used as follows:
...
[MPFR](http://www.mpfr.org/) provides correctly-rounded functions for floating-point numbers of **arbitrary precision**, including the power function. However, it can be slow.
